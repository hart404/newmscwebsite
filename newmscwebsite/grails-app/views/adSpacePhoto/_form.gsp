<%@ page import="newmscwebsite.AdSpacePhoto" %>
...
<div class="fieldcontain ${hasErrors(bean: adSpacePhotoInstance, field: 'photo', 'error')} required">
    ...
    <g:select id="photo" name="photo.id" from="${newmscwebsite.Photo.list()}" optionKey="id" required="" value="${adSpacePhotoInstance?.photo?.id}" class="many-to-one"/>
</div>
