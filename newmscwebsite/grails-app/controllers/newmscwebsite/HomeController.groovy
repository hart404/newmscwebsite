...
class HomeController {
	...
	def storeReport() {
		...
		def date = new LocalDate(dateParts[2] as Integer, dateParts[0] as Integer, dateParts[1] as Integer)
	...
	def saveFullinfoData = {
		...
		s.login = grailsApplication.config.authorize.net.api.login.identity
		s.transactionKey = grailsApplication.config.authorize.net.transaction.key
	...
	def saveTransationForShoppingCart(){
		...
		def sessionId = request.getSession().id
		def cartIdList = []
		println("sessionId  saveTransactionForShoppingCart :::::;;;"+sessionId)
